...
## [Unreleased] — バランス大改修(上方修正でビルド多様化)

単一ビルド収束(2pcスイッチ分散＋耐久職)への対策として、各ビルド概念を
**上方修正のみ**で底上げし、敵に相性(マッチアップ)多様性を追加:
...
- **敵の相性フラグ追加**(吸血無効/多段耐性/状態異常耐性/即死無効): ボス・最終
  ボスは即死無効を常時、章ボス(rank>=3)は追加で1相性。通常敵 floor>=250 は
  30%で1相性を抽選(アンデッド→状態異常耐性 など弱フレーバー優先)。単一ビルドで
  全敵に対応できなくなる。
...
- test 139件緑(新規14件: レイヤー合成/敵相性/固有共鳴2-4-6/集中ボーナス)。
