...
カジノに手続き生成武器・固有武器交換の入手導線を追加:

- **ダイス合成を非破壊レイヤー化**(`mergeEffect`): 「最後勝ち上書き」を廃止し
  職×武器×セットを「ベスト合成＋加算」で重ねる。damageMultiplierは最大値、
  isMissはどれか1つがfalseで解除、guard/extraHitsは加算、lifestealは加算で
  0.75上限、selfDamageは最大値のみ(ペナルティ非加算)。→ 攻撃/手数/固有武器を強化。
- **ボスDPS関門を緩和**: ランプ閾値を8T→12T、加速を+30%→+20%/Tに。バースト/
  グラスキャノン系でも間に合えば突破可能に。激昂/チャージは維持。
- **敵の相性フラグ追加**(吸血無効/多段耐性/状態異常耐性/即死無効): ボス・最終
  ボスは即死無効を常時。**50階区切りの大ボス(rank>=2、章ボス含む)は必ず追加1相性**
  を持つ(小ボス=rank1/10階毎は相性なし)。通常敵は**floor>=300**から**8%**で
  1相性を抽選(アンデッド→状態異常耐性 など弱フレーバー優先)。相性は壁ではなく
  希少な"スパイス"。単一ビルドへのアンチを軽く効かせる程度に頻度を絞った。
- **概念職の底上げ**: 呪術師=弱体-5＋全面に強毒(DoT成立)、盗賊=3以上で2回・6で
  3回(手数成立)、グラス火力職(魔/大魔/狂戦/武神/黒)の高火力面に8〜10%吸命を付与
  (バーストでも前進可、吸血無効が抑止役)。jobAttackMultは据え置き。
- **固有(signature)装備＋固有共鳴**: 名前付きレジェ/エピックに `signature` を付与し
  基礎攻撃を約+40〜60%、防具にHP/防を強化。新規に兜/篭手/脚甲の固有も追加し
  6部位フルビルドを成立させた。装備中の固有数から**LIVE計算**(セーブ安全)で
  攻撃%/HP%を付与: 2pc=攻+12%/HP+12%、4pc=攻+25%/追撃/リロール+1、
  6pc=攻+45%/HP+30%＋全面ノーミスの大成。★スケール後に乗算するため深層でも有効。
- **セット集中ボーナス**: 単一の名前付きセットを6部位揃えると攻撃+15%(分散より
  振り切りを競争力化、上方修正のみ)。
- DoT延命のため毒牙/疫病/業火など固有DoT武器の継続ダメ倍率を引き上げ。
- **カジノに手続き生成武器を追加**: スロットBIG賞品プールに「現在階に見合う
  手続き生成武器(`rollGenDrop`＋★mod)」を3割で混ぜ、深層ほど強い武器が当たる。
  従来のカジノ専用賞品と同じく `capInventory` 経由でインベントリへ。
- **固有武器をカジノコインで交換**(2000枚): 交換所に固有(signature)武器の交換を
  追加。`coinBuySignatureWeapon` で eligible な固有武器をランダム1つ付与
  (所持ベスト★modを継承、コイン不足ガードあり、`coinBuySouls` 同様に永続化)。
  固有武器は通常アイテム(id持ち)なのでセーブ互換に影響なし。
- test 143件緑(新規18件: レイヤー合成/敵相性/相性頻度/固有共鳴2-4-6/集中ボーナス)。

## [過去] — 全体バランス / 重さ対策 / ボス関門 / セット10種(Fusion分析)
