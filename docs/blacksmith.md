...
- **鍛える**: `forgeLevel`(0..上限)を上げる。各+1で `+7%`加算（`applyForge`）。
  成功時に **25%+で GREAT(+2) / 8%で PERFECT(+3)** の上振れ。コスト `8 + lv²×2`（〜Lv15）。
- **強化上限**: 通常は `FORGE_MAX=15`。**1000階踏破で `forgeMax(true)=99` に解放**（頭打ち防止）。
  解放後（Lv15以降）はコストを急騰させず **+80/Lv の“微増”線形**に（`forgeCost`）。鍛冶屋に踏破特典バナー＋
  メニューに「New」（`forgeUnlockSeen`）で気づける。
