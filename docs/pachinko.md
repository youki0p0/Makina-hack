...
- **リール＝ドラム式（海物語準拠）**: 3列×縦3コマ（上段/中段/下段、判定は**中段**ライン）。
  **上→下へ垂直スクロール**。停止順は **左 → 右 → 中**。フェーズ：加速(3列同時)→定速→
  順次減速(左,右)→中リールの変則アクション。`PachinkoReels.tsx` の `animate`/tween で制御。
- **テンパイ＝左右が揃う**。中リールだけ変則停止:
