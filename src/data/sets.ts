...
// Hand-tuned overrides for a few procedural indices. The generated name/icon for
// these indices is preserved so the deterministic naming (and any UI keyed on it)
// stays intact; only the bonus payload is replaced with a stronger, curated kit.
// Each override must still satisfy the procedural contract: exactly the 2/4/6 tiers
// with three distinct effect payloads (see sets.test.ts).
const PROCEDURAL_OVERRIDE_BONUSES: Record<number, SetTierBonus[]> = {
  // n=1 → 星霜の狂宴 (✶, 解放300階). 地味な攻+防セットだったのを大幅強化。
  1: [
    {
      pieces: 2,
      desc: "リロール +1 / 出目5・6の威力上昇(+40%)",
      reroll: 1,
      highFaceDmgBonus: 0.4,
    },
    {
      pieces: 4,
      desc: "攻撃時に追撃(+1ヒット) / 与ダメージの20%を吸血",
      extraHit: true,
      lifestealAllPct: 0.2,
    },
    {
      pieces: 6,
      desc: "出目6の威力上昇(+80%)＆2回発動 / ダイスを2個振り高い方 / 敵HP15%以下を即死",
      sixDmgBonus: 0.8,
      sixDouble: true,
      rollTwoDice: true,
      executePct: 0.15,
...
export function proceduralSetDef(n: number): SetDef {
  ...
  const override = PROCEDURAL_OVERRIDE_BONUSES[n];
  if (override) {
    return {
      key: `gset${n}`,
      name: `${a}の${b}`,
